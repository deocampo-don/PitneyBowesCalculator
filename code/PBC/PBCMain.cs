    ...
    {// -----------------------------
     ...
     // -----------------------------
            ...
            lvBuild.EditRequested += async (_, job) =>
            {
                ...
                using (var dialog = new CreatePBJobDialog(job))
                {
                    ...
                    try
                    {
                        ...
                        {
                            MessageDialogBox.ShowDialog("", "Job number must not be empty or contain digits only",
        ...
        private void RefreshSingleJobUI(PbJobModel job)
        {
            ...
            //SyncSelectAll();
            // Shipment view - rebuild from full _shipmentRows instead of incremental
            var shipmentRows = job.Pallets
                .Where(p => p.State == PalletState.Shipped && p.ShippedAt.HasValue)
                .GroupBy(p => new { p.ShippedAt.Value, p.JobNameSnapshot })
                .Select(group => new PbJobModel
            ...
            // ✅ Always do a full SetItems instead of incremental add/remove
        ...
        private async void btnAddPBJob_Click(object sender, EventArgs e)
        {
            ...
            using (var dlg = new CreatePBJobDialog())
            {
                ...
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(dlg.JobName))
                    {
                        MessageDialogBox.ShowDialog("", "Job name cannot be empty.",
        ...
        //            // ✅ Fix — still switch to Reports so user sees current state
        //            kryptonCheckSet1.CheckedButton = kcbPickedUp;
        //            return;
        //        }

        //        // ✅ Fix — brief delay so DB commits ShippedAt before we read it back
        ...
        private async void btnShipPallets_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Detailed change check — collect changed IDs in one pass
                var changedJobIds = new HashSet<int>();
                var changedDetails = new List<string>();
                ...
                if (changedDetails.Any())
                {
                    var msg =
                        "Some jobs were modified by another workstation:\n\n" +
                        string.Join("\n\n", changedDetails) +
                        "\n\nModified jobs will be skipped. Continue shipping the unaffected jobs?";
                ...
                // Exclude changed jobs — only ship unaffected ones
                jobIds = jobIds.Where(id => !changedJobIds.Contains(id)).ToArray();

                if (!jobIds.Any())
                {
                    MessageDialogBox.ShowDialog(
                        "Nothing to Ship",
                        "All selected jobs were modified by another workstation. No pallets were shipped.",
                        MessageBoxButtons.OK,
                        MessageType.Info);
                    return;
                }
                ...
                // Brief delay so DB commits ShippedAt before we read it back
                await Task.Delay(300);
